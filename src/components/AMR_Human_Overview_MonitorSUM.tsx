...
export function AMR_Human_Overview_MonitorSUM() {
  ...
  const fetchResistanceData = async (attempt: number = 0) => {
    try {
      ...
      console.log('Fetching resistance data from AMR_HH...');
      
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-2267887d/amr-resistance-patterns`,
        {
          method: 'GET',
          headers: {
    ...
    <div className="space-y-3">
      ...
      {!loading && !error && resistancePatterns.length === 0 && (
        <div className="flex items-center justify-center py-8 text-gray-500 text-sm">
          No resistance patterns with n≥30 total tested found
        </div>
      )}
