...
interface MDRBacteriaAllIsolatesProps {
  activeFilters: Array<{ column: string; value: string; label: string }>;
}

export function MDRBacteriaAllIsolates({ activeFilters }: MDRBacteriaAllIsolatesProps) {
  const [mdrData, setMdrData] = useState<MDRDataResponse | null>(null);
  ...
  const fetchMDRData = async () => {
    try {
      ...
      if (result.success && result.data) {
        ...
        throw new Error(result.error || 'No MDR data received');
      }
    ...
    <Card className="border border-gray-200">
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <Button
              ...
              onClick={() => {
                console.log('Download MDR bacteria all isolates data');
        ...
        <p className="text-sm text-gray-600">
          Multi-drug resistant bacteria as percentage of all isolates with valid S/I/R data • 
              ...
              <div className="pt-4 border-t border-gray-200">
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm font-medium">{mdrData.totalIsolates.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">MDR Isolates:</span>
                    <span className="text-sm font-medium text-red-600">{mdrData.mdrIsolates.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Non-MDR:</span>
                    <span className="text-sm font-medium text-green-600">{mdrData.nonMdrIsolates.toLocaleString()}</span>
                  </div>
                </div>
              </div>
            </div>
