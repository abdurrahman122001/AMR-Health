...
// Animal health isolate categories - fixed to organism only since we're showing STRAINNOTE
const animalIsolateCategories = {
  organism: {
    title: 'Animal Pathogen',
    description: 'Distribution of bacterial pathogens (STRAINNOTE) isolated from animal specimens',
    icon: '🦠'
  }
};
...
// Custom tooltip for animal isolate data
function AnimalIsolateTooltip({ active, payload }: any) {
...
// Get organism abbreviation for animal pathogens
...
export function AMR_Animal_IsolateAnalytics() {
  ...
  const currentCategory = animalIsolateCategories.organism;
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        console.log('Loading AMR_Animal isolate distribution data...');
        ...
        const url = `https://${projectId}.supabase.co/functions/v1/make-server-2267887d/amr-animal-isolate-distribution${filterQuery ? `?${filterQuery}` : ''}`;
        ...
        console.log('Received AMR_Animal data:', data);
        
        if (!data.success) {
          throw new Error(data.error || 'Failed to fetch animal isolate data');
        }
        ...
        console.log(`Loaded ${data.totalRecords} animal health records with ${processedData.length} unique pathogens`);
        ...
        if (err.name === 'AbortError' || isCancelled) {
          console.log('AMR_Animal data fetch was cancelled');
          return;
        }
        
        console.error('Error loading AMR_Animal isolate data:', err);
        setError(`Failed to load animal health isolate distribution data: ${err.message}`);
    ...
    // Small delay to debounce rapid filter changes, but much shorter
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg font-medium">
            {currentCategory.title} - Veterinary Specimen Distribution Analysis
          ...
          <div className="flex items-center gap-1">
            <Button
              ...
              onClick={() => {
                ...
                console.log(`${showTable ? 'Show chart' : 'Show table'} view for ${currentCategory.title} animal data`);
              }}
            >
            ...
            <Button
              ...
              onClick={() => {
                console.log(`Download ${currentCategory.title} animal chart data`);
      ...
      <CardContent className="pt-[0px] pr-[24px] pb-[35px] pl-[24px]">
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
            ...
            <p className="text-red-600 text-xs mt-1">
              Unable to load veterinary isolate distribution data. Please check your connection and try again.
            </p>
          </div>
        )}
              ...
              {loading ? (
                ...
                <ResponsiveContainer width={400} height={400}>
                  <PieChart>
                    ...
                    <Tooltip content={<AnimalIsolateTooltip />} />
                  </PieChart>
                </ResponsiveContainer>
              )}
            </div>
        ...
        <div className="mt-[30px] pt-4 border-t border-gray-200 mr-[0px] mb-[0px] ml-[0px]">
          <p className="text-xs text-gray-500">
            Veterinary isolate distribution analysis from animal health surveillance database. Showing {currentCategory.title.toLowerCase()} composition across {totalRecords.toLocaleString()} total veterinary isolates.
